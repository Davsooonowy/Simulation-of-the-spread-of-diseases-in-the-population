...
\begin{frame}{Etap 3: Metodologia -- 6 scenariuszy}
  ...
  \begin{tabularx}{\textwidth}{@{}lX@{}}
    ...
    \textbf{2b. Lockdown reaktywny} & \textbf{Lockdown gdy $I \geq 5\%$ pop.\ ($\approx$ dzień~13)} \\
...
\begin{frame}{Wyniki: Baseline i strategie lockdownu}
  \begin{columns}[T]
    ...
    \textbf{Sc.~1 -- Baseline:}
    \begin{itemize}
      \item Attack rate: \textbf{77,7\%};\quad Peak~I: \textbf{92} (d.~32)
    \end{itemize}

    \vspace{0.2cm}
    \textbf{Sc.~2a -- Lockdown natychmiastowy:}
    \begin{itemize}
      \item Attack rate: \textbf{4,0\%}\quad\textcolor{teal}{$\downarrow$95\%}
      \item Peak~I: \textbf{15} (dzień~7);\quad Zgony: $<1$
    \end{itemize}

    \vspace{0.2cm}
    \textbf{Sc.~2b -- Lockdown reaktywny (dzień $\approx$13):}
    \begin{itemize}
      \item Attack rate: \textbf{14,2\%}\quad\textcolor{orange}{$\downarrow$82\%}
      \item Peak~I: \textbf{36} (dzień~17);\quad Zgony: 1
    \end{itemize}

    \begin{alertblock}{Koszt opóźnienia: 13 dni = $\times$3,5 AR}
      Każdy dzień zwłoki po~przekroczeniu progu\\
      lawinowo powiększa zasięg epidemii.
    \end{alertblock}
...
\begin{frame}{Wyniki: Szczepienia i Superspreaderzy}
  \begin{columns}[T]
    ...
    \begin{itemize}
      \item Attack rate: \textbf{42,5\%}\quad\textcolor{teal}{$\downarrow$45\%}
      \item Peak~I: \textbf{37} (dzień~30)
      \item Zgony: \textbf{2}\quad\textcolor{teal}{$\downarrow$75\%}
    \end{itemize}
    {\small Prog odporności stadnej $\approx60\%$ osiągnięty,
    lecz w~grupie niezaszczepionych tworzy się aktywny klaster.}

    \vspace{0.35cm}
    \textbf{Sc.~4 -- Superspreaderzy 5\%:}
    \begin{itemize}
      \item Attack rate: \textbf{78,2\%}\quad\textcolor{red}{$\uparrow$0,5 p.p.}
      \item Peak~I: \textbf{95} (1--2 dni wcześniej)
...
\begin{frame}{Wyniki zbiorcze}
  \begin{table}
    \centering
    \caption{Mediany wyników -- 6~scenariuszy, $N=500$, 7~powtórzeń, 180~kroków}
    \scalebox{0.88}{
    \begin{tabularx}{1.14\textwidth}{@{}lYYYY@{}}
      \toprule
      \textbf{Scenariusz} & \textbf{Attack rate} & \textbf{Peak~I} &
        \textbf{Dzień} & \textbf{Zgony} \\
      \midrule
      Bazowy (brak interw.) & 77,7\% & 92  & 32 & 8 \\
      \textbf{Lockdown natychm.} & \textbf{4,0\%} & \textbf{15} & 7 & \textbf{$<$1} \\
      Lockdown reaktywny (d.13) & 14,2\% & 36 & 17 & 1 \\
      Szczepienia 60\%   & 42,5\% & 37  & 30 & 2 \\
      Superspreaderzy 5\% & 78,2\% & 95 & 35 & 8 \\
      Wysoka higiena     & 65,5\% & 65  & 34 & 7 \\
      \bottomrule
    \end{tabularx}}
  \end{table}
  ...
  \begin{columns}
    \column{0.5\textwidth}
    \textcolor{teal}{\textbf{Najskuteczniejsze:}} Lockdown natychm.\ ($-95\%$ AR)\\
    \textcolor{orange}{\textbf{Koszt 13 dni:}} Lockdown reaktywny ($\times$3,5 AR)

    \column{0.5\textwidth}
    \textcolor{teal}{\textbf{Kompromis:}} Szczepienia ($-45\%$ AR, $-75\%$ zgonów)\\
    \textcolor{red}{\textbf{Ryzyko:}} Superspreaderzy (wcześniejszy szczyt)
  \end{columns}
\end{frame}
...
\begin{frame}{Wnioski końcowe}
  \begin{enumerate}
    \item \textbf{Lockdown natychmiastowy jest najskuteczniejszy:}\\
          AR $78\% \to 4\%$ przez przecięcie wszystkich pozadomowych ścieżek.

    \vspace{0.15cm}
    \item \textbf{Koszt opóźnienia jest nieproporcjonalny:}\\
          13 dni zwłoki (lockdown reaktywny, dzień~13) $\Rightarrow$ AR wzrasta
          $\times$3,5 (do~14\%). Każdy dzień opóźnienia lawinowo powiększa zasięg.

    \vspace{0.15cm}
    \item \textbf{Szczepienia chronią głównie przed zgonem:}\\
          60\% pokrycia $\to$ $-45\%$ AR, $-75\%$ zgonów. Klaster w~grupie
    ...
    \item \textbf{Higiena spłaszcza krzywą, ale nie eliminuje epidemii:}\\
          $\Delta\Theta_H=+0{,}35$ $\Rightarrow$ $-19$~p.p.\ AR. Niewystarczające
          samodzielnie przy $R_0>1$.
  \end{enumerate}
