...
Etap trzeci stanowi właściwą część badawczą projektu: sześć scenariuszy
...
zgonów. Rozszerzeniem względem pierwotnego planu jest dodanie scenariusza
\textit{lockdownu reaktywnego}, wprowadzonego nie~od~pierwszego dnia,
lecz~dopiero po~przekroczeniu progowego poziomu zakażeń -- co~pozwala
ilościowo zbadać koszt opóźnionej reakcji służb publicznych.
...
Wyniki: attack rate~$\approx 78\%$, szczyt zakaźnych ($I_{\max} \approx 92$)
osiągany ok.\ dnia~32, zgony~$\approx 8$.
...
Wyniki: attack rate spada do~$\approx 4{,}0\%$ -- redukcja o~95\%
względem scenariusza bazowego. Szczyt zakaźnych $I_{\max} \approx 15$,
...
Wyniki: lockdown aktywował się średnio w~dniu~$\approx 13$, gdy epidemia
zdążyła już zainfekować część populacji. Attack rate wyniosła~$\approx 14\%$
(redukcja o~82\% vs.\ bazowy, lecz trzykrotnie więcej niż przy lockdownie
natychmiastowym), szczyt zakaźnych $I_{\max} \approx 36$ (dzień~17),
zgony~$\approx 1$.
...
Opóźnienie zaledwie $\approx 13$~dni powoduje wzrost attack rate z~4\%
do~14\% -- ilościowy dowód, że~wczesna reakcja ma~decydujące znaczenie.
...
\begin{figure}[!htbp]
  ...
  \caption{Porównanie strategii lockdownu: brak / reaktywny (aktywacja
           dzień~$\approx 13$, gdy $I \geq 25$) / natychmiastowy (od~dnia~0).
           Opóźnienie 13~dni zwiększa attack rate 3,5-krotnie.}
...
Wyniki: attack rate~$\approx 43\%$ (redukcja o~45\%),
szczyt zakaźnych~$I_{\max} \approx 37$, zgony~$\approx 2$.
...
głównie przed zgonem (redukcja o~75\%), nie~zaś w~całości przed
zakażeniem.
...
\begin{tabularx}{\columnwidth}{@{}lYYYY@{}} \toprule
  ...
  Bazowy (brak interwencji) & 77{,}7\% & 92 & 32 & 8 \\
  Lockdown natychmiastowy (d.~0) & 4{,}0\% & 15 & 7 & $<1$ \\
  Lockdown reaktywny (d.~$\approx$13) & 14{,}2\% & 36 & 17 & 1 \\
  Szczepienia (60\%) & 42{,}5\% & 37 & 30 & 2 \\
  Superspreaderzy (5\%) & 78{,}2\% & 95 & 35 & 8 \\
  Wysoka higiena ($\bar{\Theta}_H$=0,85) & 65{,}5\% & 65 & 34 & 7 \\
  \bottomrule
\end{tabularx}
...
\begin{figure}[!htbp]
  ...
  \caption{Krzywe SEIRD dla sześciu scenariuszy Stage~3 (siatka 2$\times$3).
...
\begin{figure}[!htbp]
  ...
  \caption{Porównanie kluczowych wskaźników: szczyt zakaźnych,
           attack rate i~liczba zgonów dla sześciu scenariuszy.
...
Wyniki eksperymentów prowadzą do~pięciu wniosków epidemiologicznych:

\begin{enumerate}
  \item \textbf{Lockdown natychmiastowy jest najskuteczniejszą interwencją}:
        redukcja attack rate z~78\% do~4\% -- efekt wynikający z~przecięcia
        wszystkich pozadomowych ścieżek transmisji od~pierwszego dnia.
  \item \textbf{Koszt opóźnienia lockdownu jest nieproporcjonalnie wysoki}:
        opóźnienie zaledwie $\approx 13$~dni (do~przekroczenia progu $I \geq 5\%$)
        powoduje wzrost attack rate z~4\% do~14\% -- trzykrotny wzrost.
        Lockdown reaktywny wciąż redukuje AR o~82\% vs.\ brak interwencji,
        ale jego skuteczność spada wykładniczo z~każdym dniem zwłoki.
  \item \textbf{Szczepienia chronią przede wszystkim przed zgonem}:
        przy 60\% pokryciu attack rate spada do~$\approx 43\%$,
        ale zgony redukowane są o~75\%. W~populacji niezaszczepionych
  ...
  \item \textbf{Higiena rąk spłaszcza krzywą, ale nie~eliminuje epidemii}:
        ...
        attack rate o~$\approx 19$~p.p.\ i~opóźnia szczyt o~$\approx 10$~dni.
