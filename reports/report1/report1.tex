...
Projekt zrealizowano w~trzech etapach: (1)~zaprojektowanie architektury
i~prototyp modelu SEIRD na~siatce, (2)~pełna implementacja heterogenicznego
środowiska miejskiego opartego na~węzłach POI (dom, szkoła, praca), mechanizmów
transmisji oraz interaktywnego interfejsu użytkownika wraz z~kalibracją
i~testami, (3)~właściwy eksperyment badawczy porównujący siedem scenariuszy
interwencji epidemicznych. Model zaimplementowano w~Pythonie z~użyciem
frameworku Mesa; eksperymenty są w~pełni odtwarzalne (stałe ziarna losowe).
Współczynnik reprodukcji wyznaczono empirycznie dwiema niezależnymi metodami
(odtworzenie krzywej $R_e(t)$ oraz relacja rozmiaru końcowego epidemii),
otrzymując zgodne $R_0 \approx 1{,}9$, na~dolnej granicy zakresu typowego dla
pandemicznych wirusów oddechowych. Eksperyment wykazał m.in., że~lockdown
natychmiastowy redukuje attack rate z~$\approx 80\%$ do~$\approx 3{,}5\%$,
że~zaledwie $\approx 9$-dniowe opóźnienie reakcji zwiększa zasięg epidemii
blisko czterokrotnie, oraz że~realistyczny pakiet łagodnych interwencji
(maseczki, higiena, częściowe szczepienia) tłumi epidemię niemal tak skutecznie
jak lockdown reaktywny, lecz bez zamykania miejsc pracy.

\noindent\textbf{Słowa kluczowe:} modelowanie agentowe, epidemiologia obliczeniowa,
model SEIRD, efektywne $R_e(t)$, transmisja chorób, Mesa, Python
\end{abstract}
...
W~docelowej implementacji (Etap~2, plik \texttt{transmission.py}) wzór ten
rozszerzono o~indywidualny ładunek wirusowy $v_i$ każdego zakaźnego agenta~$i$
oraz odporność podatnego agenta, dając efektywne prawdopodobieństwo zakażenia
w~jednym kroku:

\begin{equation}
  P_{\text{inf}} = \Bigl(1 - \prod_{i \in \mathcal{C}} (1 - p_{\text{eff},i})^{v_i}\Bigr)
                   \cdot (1 - \text{immunity})
  \label{eq:poi-inf-impl}
\end{equation}

\noindent gdzie $\mathcal{C}$ to zbiór (co~najwyżej \texttt{MAX\_INFECTIOUS\_CONTACTS},
zob.\ Sekcja~\ref{sec:transmission-impl}) wylosowanych zakaźnych sąsiadów,
$p_{\text{eff},i}$~-- prawdopodobieństwo zmodyfikowane przez maseczki, dystans
i~higienę (wzory~\eqref{eq:mask} i~\eqref{eq:hygiene-factor}), a~$v_i \in [0,1]$~--
ładunek wirusowy agenta~$i$. Człon $(1-\text{immunity})$ obniża ryzyko
u~osób zaszczepionych lub po~przebytej infekcji.
...
$p_{\text{mult}} = 0{,}16$, co~prowadzi do~attack~rate~$\approx 76$--$80\%$
przy braku interwencji. Odpowiadający temu współczynnik reprodukcji
wyznaczono empirycznie (Sekcja~\ref{sec:rt}) na~$R_0 \approx 1{,}9$ -- wartość
na~dolnej granicy zakresu typowego dla pandemicznych wirusów
oddechowych~\cite{anderson1992infectious}. Transmisję tranzytową wyłączono
(\texttt{p\_transit}~$=0$) po~stwierdzeniu, że~model grupowania tras
w~pierwotnej implementacji łączył wszystkich pasażerów podróżujących
do~danego \emph{typu} POI (zamiast do~tego samego konkretnego węzła),
co~powodowało artefaktualny R0~$\gg~10$.

\paragraph{Czasy trwania faz.} W~zaimplementowanym modelu czasy przejść
$E \to I$ oraz $I \to R/D$ pozostawiono jako wartości stałe
($4$~i~$8$~dni) -- świadome uproszczenie ułatwiające kalibrację i~testowanie.
Zapowiadane w~Etapie~1 losowanie tych czasów z~rozkładu Gamma
(Tabela~\ref{tab:pathogen}) nie~zostało wdrożone; wprowadzenie naturalnej
wariancji biologicznej traktujemy jako kierunek dalszych prac
(Sekcja~\ref{sec:limitations}).
...
Etap trzeci stanowi właściwą część badawczą projektu: siedem scenariuszy
...
zgonów. Rozszerzeniem względem pierwotnego planu są dwa dodatkowe scenariusze:
\textit{lockdown reaktywny}, wprowadzony nie~od~pierwszego dnia, lecz~dopiero
po~przekroczeniu progowego poziomu zakażeń (koszt opóźnionej reakcji służb
publicznych), oraz \textit{interwencje łączone} -- realistyczny pakiet
łagodnych środków stosowanych jednocześnie. Dodatkowo dla każdego scenariusza
wyznaczono empiryczny przebieg efektywnego współczynnika reprodukcji
$R_e(t)$ (Sekcja~\ref{sec:rt}).

\subsection{Metodologia eksperymentu}
\label{sec:exp-methodology}

Każdy scenariusz uruchomiono $n = 7$~razy, aby zminimalizować stochastyczną
wariancję wyników. Dla zapewnienia pełnej \textbf{odtwarzalności} każdy bieg
korzysta ze~stałego ziarna losowego (\texttt{seed}~$=$~\texttt{12345}~$+$~indeks
biegu), co~gwarantuje identyczne wyniki przy każdym uruchomieniu eksperymentu.
Parametry bazowe symulacji: $N = 500$ agentów, inkubacja~4~dni,
zakaźność~8~dni, $p_{\text{death}} = 0{,}02$, $p_{\text{mult}} = 0{,}16$,
...
\subsection{Empiryczny współczynnik reprodukcji $R_e(t)$}
\label{sec:rt}

Zamiast jedynie zakładać wartość $R_0$, wyznaczono ją \emph{empirycznie}
z~przebiegu symulacji. Efektywny współczynnik reprodukcji $R_e(t)$ -- średnią
liczbę wtórnych zakażeń wywołanych przez jednego zakaźnego agenta w~dniu~$t$ --
oszacowano z~zależności:

\begin{equation}
  R_e(t) \approx \frac{\text{inc}(t)}{I(t)} \cdot T_I,
  \qquad \text{inc}(t) = S(t-1) - S(t)
  \label{eq:rt}
\end{equation}

\noindent gdzie $\text{inc}(t)$ to dzienna zapadalność (liczba nowych zakażeń;
ponieważ kompartment~$S$ maleje wyłącznie na~rzecz~$E$, jest ona równa spadkowi
liczebności~$S$), $I(t)$~-- liczba aktywnie zakaźnych, a~$T_I = 8$~dni to~czas
zakaźności. Wartości wygładzono 9-dniową średnią ruchomą i~maskowano dla
$I < 5$ (eliminacja szumu przy wygasaniu epidemii).

Rysunek~\ref{fig:stage3-rt} przedstawia $R_e(t)$ dla kluczowych scenariuszy.
Uśredniając $R_e(t)$ w~fazie wczesnej (przed istotnym wyczerpaniem populacji
podatnej) dla scenariusza bazowego, otrzymano \textbf{empiryczne
$R_0 \approx 1{,}9$}. Wartość ta jest spójna z~drugą, niezależną metodą --
relacją rozmiaru końcowego epidemii $\text{AR} = 1 - e^{-R_0 \cdot \text{AR}}$,
która dla attack~rate~$\approx 80\%$ daje $R_0 \approx 2{,}0$. Zgodność dwóch
metod potwierdza, że~model operuje w~dolnej części zakresu pandemicznych
wirusów oddechowych ($R_0 \in [2{,}0; 3{,}5]$).

Wykres ujawnia mechanikę poszczególnych interwencji: w~scenariuszu bazowym
$R_e(t)$ utrzymuje się powyżej~1 aż do~ok.\ dnia~33 (wtedy wyczerpanie
podatnych samoistnie zbija transmisję); lockdown natychmiastowy trzyma
$R_e(t)$ trwale poniżej~$0{,}5$; lockdown reaktywny załamuje $R_e(t)$ poniżej~1
dokładnie w~momencie aktywacji (dzień~$\approx 9$); interwencje łączone od~startu
utrzymują $R_e(t) \lesssim 1$. W~scenariuszu szczepień widoczny jest późny pik
wtórny $R_e(t)$, odpowiadający ognisku w~grupie nieszczepionych.

\begin{figure}[!htbp]
  \centering
  \includegraphics[width=\columnwidth]{stage3_rt.png}
  \caption{Empiryczny efektywny współczynnik reprodukcji $R_e(t)$ dla
           wybranych scenariuszy (mediana 7~powtórzeń). Linia przerywana --
           próg epidemiczny $R_e = 1$; pionowa kropkowana -- moment aktywacji
           lockdownu reaktywnego. Wczesnofazowe $R_0 \approx 1{,}9$.}
  \label{fig:stage3-rt}
\end{figure}
...
Wyniki: attack rate~$\approx 79{,}7\%$, szczyt zakaźnych ($I_{\max} \approx 91$)
osiągany ok.\ dnia~34, zgony~$\approx 7$.
Empiryczne $R_0 \approx 1{,}9$ (Sekcja~\ref{sec:rt}), na~dolnej granicy
zakresu dla wirusów oddechowych~\cite{anderson1992infectious}.

\subsection{Scenariusz 2a: Lockdown natychmiastowy}
\label{sec:sc-lockdown-imm}

Pełny lockdown aktywowany od~dnia~0 symulacji: zamknięcie wszystkich miejsc
pracy (szkoły~+~biura) i~sklepów. Agenci pozostają w~gospodarstwach
domowych; parki i~placówki zdrowotne pozostają otwarte.

Wyniki: attack rate spada do~$\approx 3{,}5\%$ -- redukcja o~ponad~95\%
...
\subsection{Scenariusz 2b: Lockdown reaktywny}
\label{sec:sc-lockdown-react}

Scenariusz ten modeluje \textit{opóźnioną} reakcję służb publicznych:
lockdown nie~jest wprowadzany prewencyjnie, lecz dopiero po~przekroczeniu
progowego poziomu zakażeń $I_{\text{thr}} = 25$ agentów (5\%~populacji).
Jest to~model decyzji opartej na~danych obserwacyjnych -- analogicznie do~sytuacji,
gdy rząd ogłasza lockdown po~pierwszym potwierdzeniu istotnego wzrostu hospitalizacji.

Implementacja: przed każdym krokiem symulacji zliczana jest aktualna
liczba agentów w~stanie~I; gdy po~raz~pierwszy przekroczy~$I_{\text{thr}}$,
ustawiany jest atrybut \texttt{model.lockdown~=~True}, co~blokuje
miejsca pracy i~sklepy od~następnego dnia symulacji.

Wyniki: lockdown aktywował się średnio w~dniu~$\approx 9$, gdy epidemia
zdążyła już zainfekować część populacji. Attack rate wyniosła~$\approx 13{,}2\%$
(redukcja o~$\approx 83\%$ vs.\ bazowy, lecz blisko czterokrotnie więcej niż
przy lockdownie natychmiastowym), szczyt zakaźnych $I_{\max} \approx 38$
(dzień~13), zgony~$\approx 2$.

Rysunek~\ref{fig:stage3-lockdown} zestawia krzywe dla trzech wariantów:
brak interwencji, lockdown reaktywny i~lockdown natychmiastowy.
Opóźnienie zaledwie $\approx 9$~dni powoduje wzrost attack rate z~3{,}5\%
do~13\% -- ilościowy dowód, że~wczesna reakcja ma~decydujące znaczenie.

\begin{figure}[!htbp]
  \centering
  \includegraphics[width=\columnwidth]{stage3_lockdown_compare.png}
  \caption{Porównanie strategii lockdownu: brak / reaktywny (aktywacja
           dzień~$\approx 9$, gdy $I \geq 25$) / natychmiastowy (od~dnia~0).
           Opóźnienie $\approx 9$~dni zwiększa attack rate blisko czterokrotnie.}
  \label{fig:stage3-lockdown}
\end{figure}

\subsection{Scenariusz 3: Kampania szczepień -- 60\% pokrycia}
\label{sec:sc-vaccination}

Zaszczepiono 60\% populacji przed~startem epidemii. Modelowana szczepionka
jest \emph{nieszczelna} (\textit{leaky}): obniża podatność o~połowę
(\texttt{immunity}~$=0{,}5$) i~redukuje ryzyko zgonu o~80\%, ale nie~daje
pełnej sterylizującej ochrony. Teoretyczny próg odporności stadnej dla
$R_0 \approx 1{,}9$ wynosi $1 - 1/R_0 \approx 47\%$, jednak dotyczy on
odporności \emph{pełnej} -- przy ochronie częściowej efektywna liczba
uodpornionych jest znacznie niższa niż~60\%.

Wyniki: attack rate~$\approx 52{,}6\%$ (redukcja o~$\approx 34\%$),
szczyt zakaźnych~$I_{\max} \approx 49$, zgony~$\approx 3{,}6$
(redukcja o~$\approx 51\%$). Mimo wysokiego pokrycia epidemia nie~zostaje
zatrzymana -- w~grupie niezaszczepionych tworzy się lokalny klaster
transmisji (widoczny jako późny pik $R_e(t)$ na~Rys.~\ref{fig:stage3-rt}).
Kluczowy wniosek: nieszczelna szczepionka chroni przede wszystkim
przed~zgonem, w~mniejszym stopniu przed samym zakażeniem.
...
Wyniki: attack rate~$\approx 79{,}3\%$ (praktycznie równy bazowemu),
szczyt zakaźnych $I_{\max} \approx 100$ (vs.\ 91 w~bazowym, $+10\%$),
zgony~$\approx 9$. Na~pierwszy rzut oka efekt superspreaderów wydaje się
znikomy, jednak to~artefakt \emph{efektu sufitu}: scenariusz bazowy jest
już niemal nasycony (attack~rate~$\approx 80\%$), więc nie~ma ,,miejsca''
na~wzrost zasięgu. Rzeczywisty wpływ superspreaderów ujawnia się w~innych
miarach: \textbf{wyższy i~ostrzejszy szczyt} ($I_{\max}$ o~10\% większy)
oraz większa liczba zgonów. Mała grupa (25~agentów na~500) napędza
nieproporcjonalnie dużą część transmisji, zgodnie z~zasadą~80/20
w~epidemiologii~\cite{lloyd2001realistic}. Pełne ujawnienie tego efektu
wymagałoby reżimu nienasyconego (np.\ przy umiarkowanych interwencjach)
-- co~odnotowujemy jako kierunek dalszych prac (Sekcja~\ref{sec:limitations}).
...
\begin{itemize}
  \item niska higiena ($\bar{\Theta}_H = 0{,}15$): attack~rate~$\approx 83{,}7\%$,
        $I_{\max} \approx 118$, szczyt dzień~30,
  \item higiena bazowa ($\bar{\Theta}_H = 0{,}50$): attack~rate~$\approx 76{,}1\%$,
        $I_{\max} \approx 91$, szczyt dzień~35,
  \item wysoka higiena ($\bar{\Theta}_H = 0{,}85$): attack~rate~$\approx 64{,}4\%$,
        $I_{\max} \approx 66$, szczyt dzień~39.
\end{itemize}

Wzrost $\bar{\Theta}_H$ z~0,15 do~0,85 obniża attack rate
o~$\approx 19$~p.p., redukuje szczyt zakaźnych o~ponad~40\%
i~opóźnia go~o~$\approx 9$~dni. Wysoka higiena samodzielnie
nie~eliminuje epidemii ($R_e > 1$ nadal we~wczesnej fazie),
lecz wyraźnie spłaszcza krzywą zakaźnych (ang.\ \textit{flatten the curve}).

\subsection{Scenariusz 6: Interwencje łączone}
\label{sec:sc-combined}

Wszystkie poprzednie scenariusze badały interwencje w~izolacji. Scenariusz~6
modeluje \textit{realistyczny pakiet} łagodnych środków stosowanych
jednocześnie -- tak, jak wygląda rzeczywista polityka zdrowia publicznego:
50\% populacji w~maseczkach, podniesiona higiena ($\bar{\Theta}_H = 0{,}70$)
oraz umiarkowane pokrycie szczepieniami (40\%). Żaden z~tych środków
z~osobna nie~jest szczególnie silny.

Wyniki: attack rate~$\approx 11{,}7\%$ (redukcja o~$\approx 85\%$
względem bazowego), szczyt zakaźnych $I_{\max} \approx 16$ (dzień~11),
zgony~$< 1$. Efekt jest \textbf{silnie nieliniowy} (synergiczny):
złożenie trzech łagodnych interwencji daje skuteczność porównywalną
z~lockdownem reaktywnym (attack~rate~$13{,}2\%$), lecz \emph{bez} zamykania
miejsc pracy, szkół i~sklepów. Krzywa $R_e(t)$ (Rys.~\ref{fig:stage3-rt})
od~początku utrzymuje się poniżej~1, co~oznacza wygaszanie epidemii
od~pierwszego dnia. To~najważniejszy wniosek praktyczny eksperymentu:
skoordynowany pakiet umiarkowanych środków może zastąpić drakoński lockdown
przy znikomym koszcie społeczno-gospodarczym.

\subsection{Wyniki zbiorcze}
\label{sec:exp-results}

Tabela~\ref{tab:stage3-results} zestawia mediany wszystkich miar
dla siedmiu scenariuszy. Rysunek~\ref{fig:stage3-comparison} prezentuje
...
\caption{Zbiorcze wyniki eksperymentów Stage~3
         ($N=500$, 7~powtórzeń, 180~kroków, ziarno=12345)}
\begin{tabularx}{\columnwidth}{@{}lYYYY@{}} \toprule
  ...
  Bazowy (brak interwencji) & 79{,}7\% & 91 & 34 & 7 \\
  Lockdown natychmiastowy (d.~0) & 3{,}5\% & 14 & 8 & $<1$ \\
  Lockdown reaktywny (d.~$\approx$9) & 13{,}2\% & 38 & 13 & 2 \\
  Szczepienia (60\%) & 52{,}6\% & 49 & 31 & 4 \\
  Superspreaderzy (5\%) & 79{,}3\% & 100 & 35 & 9 \\
  Wysoka higiena ($\bar{\Theta}_H$=0,85) & 61{,}7\% & 63 & 43 & 7 \\
...
\begin{figure}[!htbp]
  ...
  \caption{Krzywe SEIRD dla siedmiu scenariuszy Stage~3 (siatka 2$\times$4).
           Linie ciągłe -- mediana 7~powtórzeń; pasma -- IQR.
           Pionowa przerywana w~Sc.~2b oznacza moment aktywacji lockdownu.}
...
\begin{figure}[!htbp]
  ...
  \caption{Porównanie kluczowych wskaźników: szczyt zakaźnych,
           attack rate i~liczba zgonów dla siedmiu scenariuszy.
...
\subsection{Obciążenie systemu ochrony zdrowia}
\label{sec:hospital}

Sama wartość attack rate nie~oddaje pełnego kosztu epidemii -- równie istotne
jest, czy~\emph{tempo} napływu chorych nie~przekracza wydolności systemu
ochrony zdrowia. Aby zilustrować klasyczny argument ,,spłaszczania krzywej''
(\textit{flatten the curve}), nałożono na~wyniki prostą warstwę pojemności:
przyjęto, że~odsetek $h = 15\%$ aktywnie zakaźnych wymaga w~danym dniu łóżka
szpitalnego, a~system dysponuje $C = 8$~łóżkami (wartość ilustracyjna dla
$N=500$). Dzienne zapotrzebowanie i~jego przekroczenie wynoszą:

\begin{equation}
  \text{popyt}(t) = h \cdot I(t), \qquad
  \text{przeciążenie}(t) = \max\bigl(0,\; h \cdot I(t) - C\bigr).
  \label{eq:hosp}
\end{equation}

\noindent Całkowitą miarą skali \emph{i}~czasu trwania kryzysu jest
\textbf{liczba osobodni ponad pojemnością} -- całka przeciążenia po~czasie,
$\Omega = \sum_t \text{przeciążenie}(t)$ (pole nad linią pojemności
na~Rys.~\ref{fig:stage3-hospital}).

\begin{table}[!htbp]
\centering
\caption{Obciążenie szpitala wg scenariusza ($C=8$ łóżek, $h=15\%$)}
\begin{tabularx}{\columnwidth}{@{}lYY@{}} \toprule
  \textbf{Scenariusz} & \textbf{Szczyt popytu} & \textbf{Osobodni $>$ pojemn.} \\ \midrule
  Bazowy (brak interwencji)   & 13,7 & 82,3 \\
  Superspreaderzy (5\%)       & 15,1 & \textbf{104,4} \\
  Wysoka higiena (0,85)       & 9,5  & 15,5 \\
  Szczepienia (60\%)          & 7,4  & 1,0  \\
  Lockdown reaktywny          & 5,7  & 0,0  \\
  Lockdown natychmiastowy     & 2,1  & 0,0  \\
  Interwencje łączone         & 2,4  & 0,0  \\
  \bottomrule
\end{tabularx}
\label{tab:hospital}
\end{table}

Wyniki (Tab.~\ref{tab:hospital}) prowadzą do~trzech obserwacji:

\begin{itemize}
  \item \textbf{Najmocniejsze ujawnienie roli superspreaderów.} Choć~ich
        attack rate jest praktycznie równy bazowemu (efekt sufitu,
        Sekcja~\ref{sec:sc-super}), to~w~obciążeniu szpitala dają
        \emph{najgorszy} wynik -- 104~osobodni vs.\ 82 w~bazowym (o~$\approx 27\%$
        więcej). Ostrzejszy, wyższy szczyt przekłada się wprost na~większe
        i~dłuższe przeciążenie -- to~tu, a~nie w~attack rate, materializuje się
        koszt superspreadingu.
  \item \textbf{Wartość spłaszczania bez eliminacji.} Szczepienia obniżają
        attack rate ,,tylko'' o~34\%, lecz spychają szczyt popytu pod~linię
        pojemności (7,4~$<$~8), redukując osobodni przeciążenia z~82 do~$\approx 1$.
        Wysoka higiena nie~eliminuje przeciążenia, ale zmniejsza je
        pięciokrotnie (82~$\to$~15,5).
  \item \textbf{Interwencje silne eliminują kryzys.} Lockdown (natychmiastowy
        i~reaktywny) oraz pakiet łączony utrzymują popyt trwale poniżej
        pojemności -- zero osobodni przeciążenia.
\end{itemize}

\begin{figure}[!htbp]
  \centering
  \includegraphics[width=\columnwidth]{stage3_hospital.png}
  \caption{Obciążenie systemu ochrony zdrowia. Lewo: dzienne zapotrzebowanie
           na~łóżka ($h \cdot I(t)$) dla wybranych scenariuszy; czerwona linia
           -- pojemność ($C=8$), zacieniony obszar -- strefa przeciążenia.
           Prawo: osobodni ponad pojemnością dla wszystkich siedmiu scenariuszy.}
  \label{fig:stage3-hospital}
\end{figure}
...
Świadoma identyfikacja ograniczeń jest elementem rzetelności pracy
modelarskiej. Najważniejsze uproszczenia obecnej implementacji oraz
wynikające z~nich kierunki rozwoju:

\begin{itemize}
  \item \textbf{Stałe czasy trwania faz.} Okresy inkubacji i~zakaźności
        są wartościami stałymi (4 i~8~dni). Wprowadzenie losowania
        z~rozkładu Gamma (zob.\ Tab.~\ref{tab:pathogen}) dodałoby naturalną
        wariancję biologiczną -- dłuższy ogon zakażeń i~bardziej miękkie
        krzywe epidemiczne.
  \item \textbf{Pojemność szpitala bez sprzężenia ze~śmiertelnością.}
        Obciążenie systemu i~osobodni ponad pojemnością wyznaczono
        (Sekcja~\ref{sec:hospital}), lecz przeciążenie nie~zwiększa jeszcze
        zwrotnie współczynnika śmiertelności. Mechanistyczne sprzężenie
        ,,brak łóżka~$\to$~wyższe CFR'' uczyniłoby koszt niekontrolowanej
        epidemii jeszcze bardziej wymiernym.
  \item \textbf{Skończony rozmiar populacji.} $N = 500$ wprowadza efekty
        skończonego rozmiaru (stochastyczne wygasanie, szum $R_e(t)$ przy
        małym~$I$). Wskazane jest badanie skalowania wyników z~$N$.
  \item \textbf{Efekt sufitu przy wysokim attack rate.} Wpływ niektórych
        czynników (np.\ superspreaderów) jest maskowany przez nasycenie
        scenariusza bazowego; ich pełna ocena wymaga reżimu nienasyconego.
  \item \textbf{Uproszczony model odporności.} Szczepionka jest nieszczelna
        i~stała w~czasie -- brak modelu zanikania odporności (\textit{waning})
        i~reinfekcji, istotnych dla dynamiki wielofalowej.
  \item \textbf{Pojedynczy patogen i~brak realnej topografii.} Architektura
        (pliki YAML, rozszerzenie \texttt{mesa-geo}) jest przygotowana
        na~wymianę patogenu i~integrację z~rzeczywistą siecią GIS/OpenStreetMap,
        co~stanowi naturalny następny krok.
  \item \textbf{Przegląd przestrzeni parametrów.} Pojedyncze punkty (60\%
        szczepień, 50\% maseczek) warto zastąpić przeglądem ciągłym (sweep),
        by~empirycznie wyznaczyć np.\ próg pokrycia szczepień dławiący epidemię.
\end{itemize}
...
i~rozszerzalna. (Docelowy model węzłowy z~Etapu~2/3 ma~odmienną topologię
kontaktów, stąd jego niezależnie zmierzone $R_0 \approx 1{,}9$ --
Sekcja~\ref{sec:rt} -- różni się od~wartości prototypu siatkowego.)
...
Współczynnik reprodukcji wyznaczono empirycznie dwiema niezależnymi metodami
(odtworzenie $R_e(t)$ oraz relacja rozmiaru końcowego), otrzymując zgodne
$R_0 \approx 1{,}9$. Wyniki eksperymentów prowadzą do~sześciu wniosków
epidemiologicznych:

\begin{enumerate}
  \item \textbf{Lockdown natychmiastowy jest najskuteczniejszą pojedynczą
        interwencją}: redukcja attack rate z~$\approx 80\%$ do~$3{,}5\%$ --
        efekt przecięcia wszystkich pozadomowych ścieżek transmisji
        od~pierwszego dnia ($R_e(t)$ trwale poniżej~$0{,}5$).
  \item \textbf{Koszt opóźnienia lockdownu jest nieproporcjonalnie wysoki}:
        opóźnienie zaledwie $\approx 9$~dni (do~przekroczenia progu $I \geq 5\%$)
        powoduje wzrost attack rate z~$3{,}5\%$ do~$13\%$ -- blisko czterokrotny.
        Każdy dzień zwłoki lawinowo powiększa zasięg.
  \item \textbf{Pakiet łagodnych interwencji może zastąpić lockdown}:
        połączenie maseczek (50\%), higieny i~częściowych szczepień (40\%)
        redukuje attack rate do~$\approx 12\%$ -- niemal tyle, co~lockdown
        reaktywny, lecz \emph{bez} zamykania gospodarki. Efekt jest
        synergiczny (nieliniowy), niewidoczny przy badaniu środków osobno.
  \item \textbf{Nieszczelne szczepienia chronią głównie przed zgonem}:
        przy 60\% pokryciu attack rate spada do~$\approx 53\%$,
        ale zgony redukowane są o~$\approx 51\%$. W~populacji niezaszczepionych
        tworzy się klaster transmisji (późny pik $R_e(t)$).
  \item \textbf{Superspreaderzy podnoszą szczyt, lecz attack rate maskuje
        efekt sufitu}: 5\% populacji o~wysokim \texttt{contact\_rate} i~niskim
        $\Theta_H$ zwiększa $I_{\max}$ o~$\approx 10\%$; brak wzrostu attack rate
        wynika z~nasycenia scenariusza bazowego, nie~z~braku wpływu
        (zasada 80/20~\cite{lloyd2001realistic}). Ich rzeczywisty koszt
        ujawnia się w~obciążeniu szpitali -- \emph{najwyższym} ze~wszystkich
        scenariuszy (Sekcja~\ref{sec:hospital}).
  \item \textbf{Higiena rąk spłaszcza krzywą, ale nie~eliminuje epidemii}:
        przejście z~$\bar{\Theta}_H = 0{,}15$ do~$0{,}85$ obniża attack rate
        o~$\approx 19$~p.p., redukuje szczyt o~ponad~40\% i~opóźnia go~o~$\approx 9$~dni.
        Samodzielna poprawa higieny nie~wystarcza przy~$R_e > 1$.
\end{enumerate}
